...
func verify_valid_location{range_check_ptr}(locationPtr : Location*):
    const MAX_RANGE = 3
    tempvar rowIndex = locationPtr.row
    assert_nn_le(rowIndex, MAX_RANGE)

    tempvar colIndex = locationPtr.col
    assert_nn_le(colIndex, MAX_RANGE)
...
func verify_location_list{range_check_ptr}(loc_list : Location*, n_steps):
    verify_valid_location(locationPtr=loc_list)
